...
| Провайдер | Endpoint | Цена за 1M tok | Статус |
|-----------|----------|:--------------:|--------|
| SiliconFlow | api.siliconflow.com/v1/embeddings | ~$0.02 | **ЗАБЛОКИРОВАН** (HTTP 403, geo-блок с 2026-02-24) |
| DeepInfra | api.deepinfra.com/v1/openai/embeddings | $0.05 | **АКТИВНЫЙ ПРОВАЙДЕР (с 2026-02-25)**, баланс $10.00, auto-topup |
| Fireworks AI | api.fireworks.ai/inference/v1/embeddings | $0.10 | **РЕЗЕРВ**, model ID: `accounts/fireworks/models/qwen3-embedding-8b` |
| Scaleway | api.scaleway.ai/v1/embeddings | ~$0.02 | active, 1M tokens free навсегда |
| OpenRouter | openrouter.ai/api/v1/embeddings | $0.01 | active |
| Ollama (локально) | localhost:11434/api/embeddings | Бесплатно | active, нужно оборудование |
...
## Пример запроса (DeepInfra - основной провайдер)
...
|--------|:-------------:|:---------:|:-----------:|:--------:|-----------|
| **Qwen3-Embedding-8B** | **88** | 8B | 4096 | 32768 | $0.05/1M (DeepInfra) |
...
Для production-сценариев рекомендуется (актуально с 2026-02-25):
1. **Основной провайдер** - DeepInfra ($0.05/1M, баланс $10.00 с auto-topup, EU датацентры)
2. **Fallback** - Fireworks AI (те же модели, $0.10/1M, активировать если DeepInfra недоступен)
3. **Для больших объёмов** - Ollama на собственном сервере
4. **Самый дешёвый API** - OpenRouter ($0.01/1M tokens)

Почему DeepInfra, а не Scaleway:
- Scaleway остаётся в резерве (1M free tokens), но DeepInfra обеспечивает полный auto-topup
- При текущих объёмах $0.05/1M на DeepInfra = практически бесплатно при имеющемся балансе
- SiliconFlow заблокирован (HTTP 403 с 2026-02-24, geo-блок)
...
- [[infra_all_instruments/docs/neural-networks/cards/qwen3-reranker-8b__20260225120000|Qwen3-Reranker-8B (карточка)]]
